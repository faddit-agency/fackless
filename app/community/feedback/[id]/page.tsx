...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createPageMetadata({
    ...
    path: `/community/feedback/${params.id}`,
    ogType: "article",
  });
}
...
export default async function FeedbackDetailPage({ params }: Props) {
  ...
  const path = `/community/feedback/${params.id}`;
    ...
    <article className="container py-10">
      <JsonLd
        data={breadcrumbJsonLd([
          { name: "홈", path: "/" },
          { name: "피드백 게시판", path: "/community/feedback" },
      ...
      <Button asChild variant="ghost" size="sm" className="-ml-2 mb-4">
        <Link href="/community/feedback">
          <ArrowLeft className="h-4 w-4" /> 피드백 목록
        </Link>
      </Button>
        ...
        {me ? (
          ...
          <div className="rounded-xl border bg-muted/40 p-4 text-sm text-muted-foreground flex items-center justify-between">
            ...
            <Button asChild size="sm" variant="accent">
              <Link href={`/login?redirectTo=/community/feedback/${post.id}`}>
                로그인
              </Link>
            </Button>
          </div>
