...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createPageMetadata({
    ...
    path: `/community/networking/${params.id}`,
    ogType: "article",
  });
}

export default async function NetworkingDetailPage({ params }: Props) {
  const supabase = createClient();
  const { data: post } = await supabase
    .from("posts")
    .select(
  ...
  const { data: comments } = await supabase
    .from("comments")
    .select(
      "id, content, created_at, like_count, author:profiles!comments_author_id_fkey(user_id, nickname, avatar_url, role_type, is_verified_expert)",
    )
    .eq("post_id", params.id)
    .neq("status", "deleted")
  ...
  const path = `/community/networking/${params.id}`;
    ...
    <article className="container py-10">
      <JsonLd
        data={breadcrumbJsonLd([
          { name: "홈", path: "/" },
          { name: "네트워킹 게시판", path: "/community/networking" },
          { name: post.title, path },
        ])}
      />

      <Button asChild variant="ghost" size="sm" className="-ml-2 mb-4">
        <Link href="/community/networking">
          <ArrowLeft className="h-4 w-4" /> 네트워킹 목록
        </Link>
      </Button>
      ...
      <section className="mt-10 space-y-4">
        <h2 className="text-lg font-bold">
          댓글 {formatNumber(comments?.length ?? 0)}
        </h2>
        <ul className="space-y-3">
          {(comments ?? []).map((comment) => {
            const cAuthor = comment.author as unknown as
              ...
              | null;
            return (
              <li key={comment.id} className="rounded-xl border p-4 bg-card">
        ...
        {me ? (
          ...
          <div className="rounded-xl border bg-muted/40 p-4 text-sm text-muted-foreground flex items-center justify-between">
            ...
            <Button asChild size="sm" variant="accent">
              <Link href={`/login?redirectTo=/community/networking/${post.id}`}>
                로그인
              </Link>
            </Button>
          </div>
