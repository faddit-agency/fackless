...
export type NewsCategorySlug = (typeof TAG_RULES)[number]["category"] | "business";

export function classifyFashionArticle(title: string, summary: string) {
  const text = `${title} ${summary}`.toLowerCase();
  const matches = TAG_RULES.filter((rule) => text.includes(rule.keyword));
  const category = (matches[0]?.category ?? "business") as NewsCategorySlug;
