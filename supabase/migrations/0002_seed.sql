...
select null::uuid, 'news'::post_type, (select id from cat_news where slug = 'fabric-news'),
       '2026 SS 원단 시장 동향: 친환경 소재 수요 35% 증가'::text,
       'fabric-trend-2026ss'::text,
       '리사이클 폴리에스터·텐셀·오가닉 코튼 중심의 원단 수요가 전년 대비 크게 증가했습니다.'::text,
       '## 한눈에 보기\n- 동대문 원단상가 기준 리사이클 폴리 35% 수요 증가\n- 오가닉 코튼 가격 8% 인상\n- 텐셀 혼방 60수 시장 회복세\n\n## 브랜드가 알아야 할 점\n샘플 단계에서 원단 인증서를 미리 확보해두는 것이 중요합니다.'::text,
       'published'::post_status, 'public'::post_visibility
where not exists (select 1 from public.posts where slug = 'fabric-trend-2026ss')
union all
select null::uuid, 'article'::post_type, (select id from cat_article where slug = 'spec-sheet'),
...
union all
select null::uuid, 'article'::post_type, (select id from cat_article where slug = 'costing'),
...
select '패클스 표준 작업지시서 템플릿 v1'::text, '봉제 공장과 협업 시 바로 쓸 수 있는 한/영 혼용 작업지시서 템플릿입니다.'::text,
       (select id from cat_res where slug = 'spec-sheet'), 'excel'::resource_file_type, 'https://faddit.app/templates/spec-sheet-v1'::text,
       null::text, array['brand','designer']::role_type[], true
...
select '패션 브랜드 원가계산 시트 (Excel)', '원단/부자재/공임/물류/수수료까지 포함한 원가 계산 시트입니다.',
       (select id from cat_res where slug = 'costing'), 'excel'::resource_file_type, 'https://faddit.app/templates/costing-v1',
       null::text, array['brand']::role_type[], true
...
select '샘플 제작 체크리스트 (PDF)', '샘플 발주 전 반드시 확인해야 할 30개 항목 체크리스트입니다.',
       (select id from cat_res where slug = 'production-check'), 'pdf'::resource_file_type, 'https://faddit.app/templates/sample-check',
       null::text, array['brand','designer']::role_type[], true
...
select '원단 용어집 (PDF)', '브랜드 운영자가 알아야 할 원단·부자재 핵심 용어 250개를 한 권으로.',
       (select id from cat_res where slug = 'fabric'), 'pdf'::resource_file_type, 'https://faddit.app/templates/fabric-glossary',
       null::text, array['brand','designer','general']::role_type[], true
...
select null::uuid, 'question'::post_type, (select id from cat_q where slug = 'fabric-qna'),
       '40수 코튼 셔츠 원단 추천 부탁드립니다 (MOQ 100m 이하)'::text,
       '초도 100장 정도 제작할 예정인데 동대문에서 적당한 40수 코튼 브로드 원단 거래처 추천 받을 수 있을까요?'::text,
       'published'::post_status, 'public'::post_visibility, 132, 6
where not exists (
  select 1 from public.posts
  where type='question' and title='40수 코튼 셔츠 원단 추천 부탁드립니다 (MOQ 100m 이하)'
)
union all
select null::uuid, 'question'::post_type, (select id from cat_q where slug = 'factory-qna'),
       '소량 생산 가능한 봉제 공장 찾고 있습니다 (수량 50장)',
       '서울 근교에서 50~100장 수준의 소량 생산 가능한 봉제 공장 어디 추천하시나요?',
       'published'::post_status, 'public'::post_visibility, 240, 11
where not exists (
  select 1 from public.posts
